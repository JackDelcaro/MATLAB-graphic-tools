...
%                    values. rgb can be an array. hex is a cell array of
%                    chars.
% 
% * * * * * * * * * * * * * * * * * * * * 
% EXAMPLES: 
...
elseif iscell(rgb)
    assert( all(cellfun(@(x) length(x) == 3, rgb)),'rgb value must have three components in the form [r g b].');
    assert( all(cellfun(@(x) max(x)<=255 && min(x)>=0, rgb)) ,'rgb values must be on a scale of 0 to 1 or 0 to 255');
    rgb = cell2mat(rgb);
else
